...
func TestMemoryBTree_Iterator(t *testing.T) {
	...
	for iter.Rewind(); iter.Valid(); iter.Next() {
		...
		// Verify key order
		if prevKey != "" {
			require.Greater(t, currKey, prevKey)
		}
	...
	for iter.Rewind(); iter.Valid(); iter.Next() {
		...
		// Verify key order
		if prevKey != "" {
			require.Less(t, currKey, prevKey)
		}
	...
	for _, tc := range testCases {
		...
		if tc.shouldFind {
			...
			require.Equal(t, tc.expectKey, string(iter.Key()))
			require.Equal(t, testData[tc.expectKey], iter.Value())
		} else {
