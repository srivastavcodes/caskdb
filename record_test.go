...
func TestEncodeDecodeLogRecord(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			"normal", &LogRecord{
				...
				BatchId: 42,
				Expire:  uint64(now + 3600),
			},
		},
		{
			"empty_key_value", &LogRecord{
				Key:     []byte(""),
				Val:     []byte(""),
		...
		{
			"large_ids", &LogRecord{
				Key:     bytes.Repeat([]byte{'a'}, 512),
				Val:     bytes.Repeat([]byte{'b'}, 1024),
				Type:    LogRecordNormal,
				BatchId: 1 << 50,
				Expire:  uint64(now),
			},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if !bytes.Equal(got.Val, c.rec.Val) {
				t.Fatalf("Val mismatch: got %v want %v", got.Val, c.rec.Val)
			}
		})
	}
}
...
func TestEncodeDecodeHintRecord(t *testing.T) {
	...
	if gotPos.SegmentId != pos.SegmentId ||
		gotPos.BlockNumber != pos.BlockNumber ||
		gotPos.ChunkOffset != pos.ChunkOffset || gotPos.ChunkSize != pos.ChunkSize {
...
func TestEncodeMergeFinRecord(t *testing.T) {
	...
	if seg != val {
		t.Fatalf("segment id mismatch: got %x want %x", val, seg)
	}
}
...
func TestLogRecordIsExpired(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			lr := &LogRecord{Expire: uint64(c.expire)}
